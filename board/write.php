...
<nav class="navbar navbar-inverse navbar-fixed-top">
  <div class="container">
    ...
    <form method=POST name="post" action="write_ok.php" enctype="multipart/form-data">
      <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>" />
        <label for="title">제목</label>
        <div>
          <textarea cols="60" name="title" placeholder="제목"></textarea>
        </div>
        <div>
          <label for="content">게시글</label>
          <div>
            <textarea cols="60" rows="20" name="text" placeholder="글 내용"></textarea>
          </div>
        </div>
        <div id ="in_file">
          <input type="file" value="1" name="file" size=100 />
        </div>
          <div>
